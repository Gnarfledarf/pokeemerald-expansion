...
void PrintMenuActionTextsInUpperLeftCorner(u8 windowId, u8 itemCount, const struct MenuAction *menuActions, const u8 *actionIds)
{
    ...
    printer.color.foreground = GetFontAttribute(FONT_SHORT, FONTATTR_COLOR_FOREGROUND);
    printer.color.background = GetFontAttribute(FONT_SHORT, FONTATTR_COLOR_BACKGROUND);
    printer.color.shadow = GetFontAttribute(FONT_SHORT, FONTATTR_COLOR_SHADOW);
    printer.color.accent = GetFontAttribute(FONT_SHORT, FONTATTR_COLOR_ACCENT);
    printer.letterSpacing = 0;
    printer.lineSpacing = 0;
    printer.x = 8;
    printer.currentX = 8;
